...
import { createMysqlLeadOutreachRepository } from "./persistence/mysql-lead-outreach-repository.js";
...
export async function createMysqlStore(options: MysqlStoreOptions = {}): Promise<CrmStore> {
  ...
  const store: CrmStore = {
    ...
    leadOutreachRepository: createMysqlLeadOutreachRepository(pool),
...
async function persistAll(pool: mysql.Pool, store: CrmStore, vault: SecretVault) {
  ...
  try {
    ...
    // lead_outreach_requests is owned by LeadOutreachRepository and must never be snapshot-replaced.

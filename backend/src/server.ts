...
// ---------------------------------------------------------------------------
// Leads (线索管理) — modular core intake/lifecycle routes; outreach/conversion follow below
// ---------------------------------------------------------------------------
...
async function translateToChinese(user: SessionUser, text: string): Promise<string> {
  ...
  try {
    const result = (await httpModelGateway.generateText({ config, prompt, maxInputChars: 4000 })).content;
...
async function aiGenerateLeads(query: LeadQuery, config: AiModelConfig): Promise<RawLead[]> {
  ...
  const content = (await httpModelGateway.generateText({ config, prompt, maxInputChars: 4000 })).content;
...
async function parseWebsiteWithAi(config: AiModelConfig, context: {
  ...
  const content = (await httpModelGateway.generateText({ config, prompt, maxInputChars: 12000 })).content;

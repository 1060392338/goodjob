...
| R-004 | 后端 `server.ts` 与前端 `prototype-api.ts` 文件过大，修改影响面和冲突风险高 | High | Mitigating | ADR-0005 规定渐进拆分；L-0004 已迁移系统/认证 4 个 API 并新增独立路由测试与 167 操作基线；L-0005 已迁移客户 5 个 API 与客户领域服务，专项角色/级联测试和 167/37 门禁通过；后续继续线索、AI、集成和前端模块 | 阶段 2 |
